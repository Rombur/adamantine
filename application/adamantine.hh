...
dealii::Vector<float> estimate_error(
    ...
    dealii::LA::distributed::Vector<double, MemorySpaceType> const &field)
{
  unsigned int const fe_degree = dof_handler.get_fe_collection().max_degree();
  ...
  dealii::KellyErrorEstimator<dim>::estimate(
      ...
      field, estimated_error_per_cell, dealii::ComponentMask(), nullptr, 0,
...
{
  unsigned int const fe_degree = dof_handler.get_fe_collection().max_degree();
  dealii::LA::distributed::Vector<double, dealii::MemorySpace::Host> field_host(
      field.get_partitioner());
  field_host.import(field, dealii::VectorOperation::insert);
  dealii::Vector<float> estimated_error_per_cell(
      triangulation.n_active_cells());
  dealii::KellyErrorEstimator<dim>::estimate(
      dof_handler, dealii::QGauss<dim - 1>(fe_degree + 1),
      std::map<dealii::types::boundary_id,
               const dealii::Function<dim, double> *>(),
      field_host, estimated_error_per_cell, dealii::ComponentMask(), nullptr, 0,
      triangulation.locally_owned_subdomain());
        ...
        &displacement)
{
  ...
  adamantine::RefineTransferHelper<dim, MemorySpaceType> refine_transfer_helper(
      thermal_physics, mechanical_physics, material_properties, triangulation,
      temperature, displacement);
  refine_transfer_helper.execute();
}
...
void refine_mesh(
    ...
    adamantine::MaterialProperty<dim, MemorySpaceType> &material_properties,
    dealii::LA::distributed::Vector<double, MemorySpaceType> &temperature,
    dealii::LA::distributed::Vector<double, MemorySpaceType> &displacement,
    std::vector<std::shared_ptr<adamantine::HeatSource<dim>>> &heat_sources,
    double const time, double const next_refinement_time,
    ...
    boost::property_tree::ptree const &refinement_database)
{
  ...
  auto const &triangulation_from_dof =
      thermal_physics
          ? thermal_physics->get_dof_handler().get_triangulation()
          : mechanical_physics->get_dof_handler().get_triangulation();
  dealii::parallel::distributed::Triangulation<dim> &triangulation =
      dynamic_cast<dealii::parallel::distributed::Triangulation<dim> &>(
          const_cast<dealii::Triangulation<dim> &>(triangulation_from_dof));
  ...
  for (unsigned int i = 0; i < n_kelly_refinements; ++i)
  {
    ...
    if (thermal_physics)
    {
      estimated_error_per_cell = estimate_error(
          triangulation, thermal_physics->get_dof_handler(), temperature);
      if (mechanical_physics)
      {
        estimated_error_per_cell += estimate_error(
            triangulation, mechanical_physics->get_dof_handler(), temperature);
      }
    }
    else
    {
    ...
    // Don't refine cells that are already as refined as it is allowed.
    for (auto cell :
    ...
    // Execute the refinement and transfer the temperature onto the new mesh.
    refine_and_transfer(thermal_physics, mechanical_physics,
                        material_properties, triangulation, temperature,
                        displacement);
  }

  // Refine the mesh along the trajectory of the sources, it we solve mechanical
  // problem
  // TODO
  if (thermal_physics)
  {
    ...
    boost::property_tree::ptree const &refinement_database)
{
  ...
  case 1:
  {
    refine_mesh<dim, 1>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
  ...
  case 2:
  {
    refine_mesh<dim, 2>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 3:
  {
    refine_mesh<dim, 3>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 4:
  {
    refine_mesh<dim, 4>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 5:
  {
    refine_mesh<dim, 5>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 6:
  {
    refine_mesh<dim, 6>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 7:
  {
    refine_mesh<dim, 7>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 8:
  {
    refine_mesh<dim, 8>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 9:
  {
    refine_mesh<dim, 9>(thermal_physics, mechanical_physics,
                        material_properties, temperature, displacement,
                        heat_sources, time, next_refinement_time,
                        time_steps_refinement, refinement_database);
    break;
  ...
  case 10:
  {
    refine_mesh<dim, 10>(thermal_physics, mechanical_physics,
                         material_properties, temperature, displacement,
                         heat_sources, time, next_refinement_time,
                         time_steps_refinement, refinement_database);
    break;
    ...
    std::vector<adamantine::Timer> &timers)
{
  ...
  while (time < duration)
  {
        ...
        (time >= next_refinement_time))
    {
      ...
      refine_mesh(thermal_physics, mechanical_physics, material_properties,
                  temperature, displacement, heat_sources, time,
                  next_refinement_time, time_steps_refinement,
                  refinement_database);
      timers[adamantine::refine].stop();
      if ((rank == 0) && (verbose_output == true))
      {
        std::cout << "n thermal dofs: "
                  << (use_thermal_physics
                          ? thermal_physics->get_dof_handler().n_dofs()
                          : 0)
                  << std::endl;
        std::cout << "n mechanical dofs: "
                  << (use_mechanical_physics
        ...
        if (activation_start < activation_end)
        {
          if (use_thermal_physics)
          {
            ...
            thermal_physics->add_material(
                ...
                has_melted, activation_start, activation_end,
      ...
      if (use_thermal_physics)
      {
        time = thermal_physics->evolve_one_time_step(time, time_step,
                                                     temperature, timers);
      }
      ...
      if (use_mechanical_physics)
      {
        ...
        if (n_time_step % time_steps_output == 0)
        {
          if (use_thermal_physics)
          {
            ...
            dealii::LA::distributed::Vector<double, dealii::MemorySpace::Host>
                temperature_host(temperature.get_partitioner());
            temperature_host.import(temperature,
  ...
  template <int dim, typename MemorySpaceType>
  std::vector<dealii::LA::distributed::BlockVector<double>> run_ensemble(
      MPI_Comm const &communicator, boost::property_tree::ptree const &database,
      std::vector<adamantine::Timer> &timers)
  {
#ifdef ADAMANTINE_WITH_CALIPER
    ...
    std::vector<double> new_material_temperature =
        adamantine::fill_and_sync_random_vector(
            ensemble_size, new_material_temperature_mean,
    ...
    std::vector<adamantine::AugmentedStateParameters>
        augmented_state_parameters;
               ...
               index < augmented_state_parameters.size(); ++index)
          {
                ...
                adamantine::AugmentedStateParameters::beam_0_absorption)
            {
              solution_augmented_ensemble[member].block(
                  augmented_state)[index] = beam_0_absorption[member];
            }
            else if (augmented_state_parameters.at(index) ==
                     adamantine::AugmentedStateParameters::beam_0_max_power)
            {
              solution_augmented_ensemble[member].block(
                  augmented_state)[index] = beam_0_max_power[member];
            }
          }
        }
      }
    ...
    if (experiment_optional_database)
    {
      ...
      if (read_in_experimental_data)
      {
        ...
        if (rank == 0)
          std::cout << "Done. Log entries found for "
                    << frame_time_stamps.size() << " camera(s), with "
                    << frame_time_stamps[0].size() << " frame(s)." << std::endl;
    ...
    while (time < duration)
    {
          ...
          (time >= next_refinement_time))
      {
        ...
        for (unsigned int member = 0; member < ensemble_size; ++member)
        {
          ...
          refine_mesh(thermal_physics_ensemble[member], mechanical_physics,
                      *material_properties_ensemble[member],
                      solution_augmented_ensemble[member].block(base_state),
                      dummy, heat_sources_ensemble[member], time,
                      next_refinement_time, time_steps_refinement,
                      refinement_database);
          solution_augmented_ensemble[member].collect_sizes();
        }
      ...
      if (assimilate_data)
      {
        ...
        if (frame_time <= time)
        {
          ...
          bool const output_experiment_on_mesh =
              experiment_optional_database.get().get(
...
#endif
          data_assimilator.update_ensemble(communicator,
                                           solution_augmented_ensemble,
