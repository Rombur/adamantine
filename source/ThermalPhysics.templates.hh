    ...
    std::vector<Timer> &timers)
{
           ...
           dealii::IteratorFilters::ActiveFEIndexEqualTo(0)))
  {
    ...
    // If we don't have adiabatic boundary conditions, we need to add boundary
    // conditions
    if (!adiabatic_only_bc)
    {
        ...
        dealii::LA::distributed::Vector<double, MemorySpaceType> &temperature)
{
  ...
  unsigned int constexpr data_size_per_cell =
      n_material_states + direction_data_size + 1 + 1;
        ...
        dealii::LA::distributed::Vector<double, MemorySpaceType> &temperature)
{
  ...
  for (auto const &cell : _dof_handler.active_cell_iterators())
  {
    if (cell->is_locally_owned())
    {
      ...
      // Store the FE index
      data_to_serialize[cell_id][n_material_states + direction_data_size + 1] =
          fe_index;
